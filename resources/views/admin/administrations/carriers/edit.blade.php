        ...
        <form  method="POST"   action="{{ route('carriers.update',$carrier->id) }}" enctype="multipart/form-data" >
            ...
            <div class="item form-group">
                ...
                <div class="col-md-4 col-sm-4 ">
                    <input type="text" id="code" name="code" required="required" class="form-control " value="{{ $carrier->code }}">
                </div>
            </div>
            <div class="item form-group">
                <label class="col-form-label col-md-3 col-sm-3 label-align" for="name">Nombre:</label>
                <div class="col-md-4 col-sm-4 ">
                    <input type="text" id="name" name="name" required="required" class="form-control " value="{{ $carrier->name }}">
                </div>
            </div>
            <div class="item form-group">
...
@section('validation')
    <script>
        if("{{$carrier->type}}" == "Maritimo"){
            $('#typeMaritimo').prop('checked', true);
        }else if("{{$carrier->type}}" == "Aereo"){
